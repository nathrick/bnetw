...
void server::start()
{
  ...
  auto new_conn = boost::make_shared<connection<boost::asio::ip::tcp::socket>>(acceptor_.get_executor().context());
...
void server::handle_accept(const boost::system::error_code &e, connection_ptr<boost::asio::ip::tcp::socket> conn)
{
  if (!e)
  {
